  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-2xl font-bold text-gray-900 dark:text-white">Rp{totalBalance.toFixed(2)}</span>
      </div>
      <div className="space-y-4 mb-6">
        {accounts.map((account) => (
          <div key={account.name} className="flex justify-between items-center">
            ...
            <span className="font-medium text-gray-900 dark:text-white">Rp{account.balance.toFixed(2)}</span>
          </div>
        ))}
      </div>
